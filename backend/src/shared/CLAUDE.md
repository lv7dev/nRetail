...
>
> **Do NOT include `?sslmode=require` in `DATABASE_URL`** — `pg-connection-string` v3 (bundled with pg v9) treats `sslmode=require` as `verify-full`, which enforces full certificate chain verification and overrides `ssl: { rejectUnauthorized: false }` set on the Pool. This causes a `self-signed certificate in certificate chain` TLS error on Render/Supabase. Remove `?sslmode=require` from the URL and let `PrismaService` handle SSL via the Pool config. Also ensure `NODE_ENV=production` is set in Render's environment so the ssl option is applied.
