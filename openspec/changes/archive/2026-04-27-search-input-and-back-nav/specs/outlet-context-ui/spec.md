...
#### Scenario: Tapping outlet name in expanded state navigates to outlet picker with canGoBack
- **WHEN** the user taps the outlet name row in the expanded state
- **THEN** the app navigates to `/outlets` with router state `{ canGoBack: true }`

#### Scenario: Tapping outlet name in collapsed (pill) state navigates to outlet picker with canGoBack
