...
The LanguageSwitcher in AuthLayout SHALL be positioned so that its top edge is below the OS status bar on all Zalo-supported devices. The vertical offset SHALL be `calc(var(--zaui-safe-area-inset-top, 0px) + 1rem)` from the top of the viewport.

#### Scenario: LanguageSwitcher visible on device with transparent status bar
- **WHEN** the user opens any auth page on a device where `--zaui-safe-area-inset-top` is greater than `0px`
