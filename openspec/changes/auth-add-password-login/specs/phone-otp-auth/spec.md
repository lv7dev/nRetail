...
**Reason:** Auto-inserting phones into PhoneConfig on registration created confusing ownership semantics. PhoneConfig is now managed manually for per-phone OTP overrides only. The `'999999'` fallback is now unconditional (not dependent on a row existing).
**Migration:** Remove the `phoneConfigRepository.upsert()` call from the registration flow. Existing PhoneConfig rows are unaffected. To assign a custom OTP for a phone, insert a row directly into the `PhoneConfig` table.
